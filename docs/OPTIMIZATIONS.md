...
## 2. Адаптивний спліт/мердж + fill-idle-cores
...
**Fill-idle-cores (balancing):** профіль показав 28% CPU + main припаркований на
бар'єрі = робота в кількох регіонах, ядра простоюють. Коли зайнятих регіонів
менше за `cores-1`, ділимо найгарячіший на нижчому порозі (`splitFillCoresMs`=12мс)
→ навантаження розходиться на вільне ядро. Обмежено к-стю ядер (не більше потоків,
ніж ядер). Merge придушено поки регіонів ≤ цілі (анти-трешинг), але **порожні
регіони зливаються завжди**. Зайнятість рахується лише по регіонах з ентіті.
Перевірено: 360 мобів → каскад 1→8 регіонів (136→7.6мс), очищення → 8→2.
Безпечно на малих машинах (≤2 ядра → ціль=1 → не спрацьовує).
