...
/**
 * Phase 11 #31.4 step 2 measurement-only diagnostics: how much wall time {@code
 * BlockState.use()} costs on each side, main thread (fallback path) vs. region thread (the
 * dispatched path). Same reasoning as {@link PlayerAttackTiming} -- neither existing subphase
 * timer captures this cost. Not wired into any command by itself beyond {@code
 * /nestworld playerinteractiontiming[reset]}; exists purely to answer one question for a single
 * measurement test (does the workload actually move?).
 */
public final class PlayerInteractionTiming {
    ...
    private PlayerInteractionTiming() {}
